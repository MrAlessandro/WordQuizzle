...
public class TestChallengesManager
{
    ...
    public void testChallengeCanceling()
    {
        ...
        AtomicReference<ChallengeReport[]> reports = new AtomicReference<>(null);
        assertDoesNotThrow(() -> reports.set(this.challengesManager.cancelChallenge(username1)));
        if (reports.get()[0].player.equals(username1))
            assertEquals(reports.get()[1].player, username2);
        else if (reports.get()[1].player.equals(username1))
            assertEquals(reports.get()[0].player, username2);
        else
            fail("WRONG REPORTS RETURNED BY CHALLENGE CANCELLATION");
        assertEquals(0, challengesArchive.size());
    }
        ...
        public void testConcurrentChallengeCompletion(int tasksNum)
        {
            ...
            for (int i = 0; i < tasksNum; i++)
            {
                ...
                assertDoesNotThrow(() -> challengesManager.recordChallenge(username1, username2,
                        challengeReportDelegation -> completionFlags[finalI].set(true),
                        voidOperation));
